-- Use a single atomic RPC for Job Seeker profile updates to avoid partial writes when
-- child collections are replaced in the same save action.
...
begin
  ...
  if not exists (select 1 from public.profiles where id = v_user_id and user_type = 'job_seeker') then
    raise exception 'Job seeker profile not found';
  end if;

  v_full_name := coalesce(trim(v_profile->> 'first_name'), '') || case when coalesce(trim(v_profile->> 'first_name'), '') <> '' and coalesce(trim(v_profile->> 'last_name'), '') <> '' then ' ' else '' end || coalesce(trim(v_profile->> 'last_name'), '');
  ...
  set
    ...
    headline = coalesce(nullif(trim(v_profile->> 'professional_headline'), ''), nullif(trim(v_profile->> 'headline'), '')),
    ...
    years_experience = case when v_profile->>'years_experience' is null or trim(v_profile->>'years_experience') = '' then null else (v_profile->>'years_experience')::integer end,
    ...
    preferred_locations = case
      ...
      else array(select trim(value::text) from jsonb_array_elements_text(v_profile->'preferred_locations') as value where trim(value::text) <> '')
    end,
    expected_salary = case when v_profile->>'expected_salary' is null or trim(v_profile->>'expected_salary') = '' then null else (v_profile->>'expected_salary')::numeric end,
    availability = case
      when v_profile->>'availability' in ('not_specified', 'immediately', 'notice_period', 'not_available') then v_profile->>'availability'
      else 'not_specified'
    end,
  ...
  with experience_rows as (
    select jsonb_array_elements(case when p_experience is null then '[]'::jsonb else p_experience end) as item
  )
  ...
  select
    ...
    case when coalesce((item->>'is_current')::boolean, false) then null else nullif(item->>'end_date', '') end,
    coalesce((item->>'is_current')::boolean, false),
    nullif(trim(item->>'description'), '')
  from experience_rows
  where coalesce(nullif(trim(item->>'job_title'), ''), nullif(trim(item->>'company_name'), ''), nullif(trim(item->>'location'), '')) is not null;

  with education_rows as (
    select jsonb_array_elements(case when p_education is null then '[]'::jsonb else p_education end) as item
  )
  ...
  select
    ...
    nullif(trim(item->>'description'), '')
  from education_rows
  where coalesce(nullif(trim(item->>'institution'), ''), nullif(trim(item->>'degree'), ''), nullif(trim(item->>'field_of_study'), '')) is not null;

  with skill_rows as (
    select jsonb_array_elements_text(case when p_skills is null then '[]'::jsonb else p_skills end) as name
  )
  insert into public.skills (user_id, name, proficiency)
  select v_user_id, trim(name), 'Professional'
  from skill_rows
  where trim(name) <> '';

  with certification_rows as (
    select jsonb_array_elements(case when p_certifications is null then '[]'::jsonb else p_certifications end) as item
  )
  ...
  where coalesce(nullif(trim(item->>'name'), ''), nullif(trim(item->>'issuing_organization'), '')) is not null;

  with language_rows as (
    select jsonb_array_elements(case when p_languages is null then '[]'::jsonb else p_languages end) as item
  )
  ...
  with link_rows as (
    select jsonb_array_elements(case when p_links is null then '[]'::jsonb else p_links end) as item
  )
...
revoke all on function public.save_job_seeker_profile(jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb) from public;
